...
class Particle {
    ...
    draw = canvas => {
        ...
        const mouseDistance = Math.sqrt(Math.pow(p.currentX - mpX, 2) + Math.pow(p.currentY - mpY, 2))

        if (mouseDistance < p.mouseAvoidRadius) {
            // reset base speed with angle

            let avoidSpeed = p.mouseAvoidStrength * Math.pow(((p.mouseAvoidRadius - mouseDistance) / p.mouseAvoidRadius), 2)
...
export default function Starfield() {
    ...
    function setMousePosition(ev) {
        state = {...state, mousePosition: {x: ev.nativeEvent.offsetX, y: ev.nativeEvent.offsetY}}
    }

    return (
        <CanvasBox>
            <CanvasSC width={dimensions.width * 4} height={dimensions.height * 4} onMouseMove={setMousePosition} ref={canvasRef} />
        </CanvasBox>
    );
}
